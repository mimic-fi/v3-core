...
describe('SymbiosisBridger', () => {
  ...
  before('deploy connector', async () => {
    connector = await deploy('SymbiosisConnectorMock')
  ...
  describe('bridger', () => {
    ...
    itBehavesLikeBaseBridgeTask('SYMBIOSIS_BRIDGER')
  })
  ...
  describe('call', () => {
    ...
    context('when the sender is authorized', () => {
      ...
      context('when the token is not the address zero', () => {
        ...
        context('when the amount is not zero', () => {
          ...
          context('when the destination chain was set', () => {
            ...
            context('when the given token is allowed', () => {
              context('when the current balance passes the threshold', () => {
                ...
                const itExecutesTheTaskProperly = (requestedAmount: BigNumberish) => {
                  it('executes the expected connector', async () => {
                    const tx = await task.call(token.address, requestedAmount, data)
                ...
                context('with balance connectors', () => {
                  ...
                  it('updates the balance connectors properly', async () => {
                    const tx = await task.call(token.address, amount, data)
              ...
              context('when the current balance does not pass the threshold', () => {
                ...
                it('reverts', async () => {
                  await expect(task.call(token.address, amount, '0x')).to.be.revertedWith('TaskTokenThresholdNotMet')
                })
              })
            })
            ...
            context('when the given token is not allowed', () => {
              ...
              it('reverts', async () => {
                await expect(task.call(token.address, amount, '0x')).to.be.revertedWith('TaskTokenNotAllowed')
              })
            })
          })
          ...
          context('when the destination chain was not set', () => {
            it('reverts', async () => {
              await expect(task.call(token.address, amount, '0x')).to.be.revertedWith('TaskDestinationChainNotSet')
            })
          })
        })
        ...
        context('when the amount is zero', () => {
          ...
          it('reverts', async () => {
            await expect(task.call(token.address, amount, '0x')).to.be.revertedWith('TaskAmountZero')
          })
        })
      })
      ...
      context('when the token is the address zero', () => {
        ...
        it('reverts', async () => {
          await expect(task.call(token, 0, '0x')).to.be.revertedWith('TaskTokenZero')
        })
      })
    })
    ...
    context('when the sender is not authorized', () => {
      it('reverts', async () => {
        await expect(task.call(ZERO_ADDRESS, 0, '0x')).to.be.revertedWith('AuthSenderNotAllowed')
      })
    })
  })
})
