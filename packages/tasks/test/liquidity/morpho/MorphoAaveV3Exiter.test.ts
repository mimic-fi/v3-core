...
describe('MorphoAaveV3Exiter', () => {
  ...
  describe('morpho-aave V3', () => {
    ...
    itBehavesLikeBaseMorphoAaveV3Task('MORPHO_AAVE_V3_EXITER')
  })
  ...
  describe('call', () => {
    ...
    context('when the sender is authorized', () => {
      ...
      context('when the token is not zero', () => {
        let token: Contract
        const maxIterations = 0 // pass 0 to fallback to the default
          ...
          context('when the threshold has passed', () => {
            ...
            const itExecutesTheTaskProperly = (requestedAmount: BigNumberish) => {
              it('executes the expected connector', async () => {
                ...
                await assertIndirectEvent(tx, connector.interface, 'LogExit', { token, amount })
              })
        ...
        context('when the amount is zero', () => {
          ...
          it('reverts', async () => {
            await expect(task.call(token.address, amount, maxIterations)).to.be.revertedWith('TaskAmountZero')
          })
        })
      })
