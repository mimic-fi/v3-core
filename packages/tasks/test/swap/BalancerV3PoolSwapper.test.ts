...
describe('BalancerV3PoolSwapper', () => {
  let task: Contract
  let smartVault: Contract, authorizer: Contract, connector: Contract, priceOracle: Contract, owner: SignerWithAddress
  ...
  before('deploy connector', async () => {
    connector = await deploy('BalancerV3PoolConnectorMock', [ZERO_ADDRESS, ZERO_ADDRESS])
  ...
  describe('swapper', () => {
    ...
    itBehavesLikeBaseSwapTask('BALANCER_V3_POOL_SWAPPER')
  })
          ...
          context('when the token in is allowed', () => {
            context('when there is a token out set', () => {
              ...
              context('when an off-chain oracle is given', () => {
                ...
                const executeTask = async (amountIn, slippage): Promise<ContractTransaction> => {
                  const callTx = await task.populateTransaction.call(tokenIn.address, amountIn, slippage, [])
                ...
                context('when an on-chain oracle is given', () => {
                  ...
                  context('when the smart vault balance passes the threshold', () => {
                    ...
                    context('when the slippage is below the limit', () => {
                      ...
                      it('executes the expected connector', async () => {
                        const tx = await task.call(tokenIn.address, amountIn, slippage, steps)
