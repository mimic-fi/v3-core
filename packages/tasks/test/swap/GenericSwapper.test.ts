...
describe('GenericSwapper', () => {
  ...
  before('deploy connector', async () => {
    connector = await deploy('GenericSwapConnectorMock')
  ...
  describe('swapper', () => {
    ...
    itBehavesLikeBaseSwapTask('GENERIC_SWAPPER')
  })

  describe('setSwapTargets', () => {
    context('when the sender is authorized', () => {
      ...
      context('when the target is not zero', () => {
        const target = ONES_ADDRESS

        context('when allowing the target', () => {
          it('allows the requested target', async () => {
            await task.setSwapTargets([target], [true])

            expect(await task.isTargetAllowed(target)).to.be.true
          })

          it('emits an event', async () => {
            const tx = await task.setSwapTargets([target], [true])

            await assertEvent(tx, 'SwapTargetSet', { target, allowed: true })
          })
        })

        context('when disallowing the target', () => {
          beforeEach('allow swap target', async () => {
            await task.setSwapTargets([target], [true])
          })

          it('allows the requested target', async () => {
            await task.setSwapTargets([target], [false])

            expect(await task.isTargetAllowed(target)).to.be.false
          })

          it('emits an event', async () => {
            const tx = await task.setSwapTargets([target], [false])

            await assertEvent(tx, 'SwapTargetSet', { target, allowed: false })
          })
        })
      })

      context('when the target is zero', () => {
        const target = ZERO_ADDRESS

        it('reverts', async () => {
          await expect(task.setSwapTargets([target], [true])).to.be.revertedWith('TaskSwapTargetZero')
        })
      })
    })
    ...
    context('when the sender is not authorized', () => {
      it('reverts', async () => {
        await expect(task.setSwapTargets([ZERO_ADDRESS], [true])).to.be.revertedWith('AuthSenderNotAllowed')
      })
    })
  })
