...
/**
 * @title OKX DEX connector interface
 */
interface IOkxConnector {
    /**
     * @dev The token in is the same as the token out
     */
    error OkxSwapSameToken(address token);

    /**
     * @dev The amount out is lower than the minimum amount out
     */
    error OkxBadAmountOut(uint256 amountOut, uint256 minAmountOut);

    /**
     * @dev The post token in balance is lower than the previous token in balance minus the amount in
     */
    error OkxBadPostTokenInBalance(uint256 postBalanceIn, uint256 preBalanceIn, uint256 amountIn);

    /**
     * @dev Tells the reference to OKX DEX swapper
     */
    function dexRouter() external view returns (address);

    /**
     * @dev Tells the reference to Token Approve contract
     */
