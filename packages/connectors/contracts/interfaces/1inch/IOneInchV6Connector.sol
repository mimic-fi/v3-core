...
/**
 * @title 1inch V6 connector interface
 */
interface IOneInchV6Connector {
    /**
     * @dev The token in is the same as the token out
     */
    error OneInchV6SwapSameToken(address token);

    /**
     * @dev The amount out is lower than the minimum amount out
     */
    error OneInchV6BadAmountOut(uint256 amountOut, uint256 minAmountOut);

    /**
     * @dev The post token in balance is lower than the previous token in balance minus the amount in
     */
    error OneInchV6BadPostTokenInBalance(uint256 postBalanceIn, uint256 preBalanceIn, uint256 amountIn);

    /**
     * @dev Tells the reference to 1inch aggregation router v6
     */
    function oneInchV6Router() external view returns (address);
