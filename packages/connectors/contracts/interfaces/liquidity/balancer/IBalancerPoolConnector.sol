 ...
 */
interface IBalancerPoolConnector {
    /**
     * @dev The given input length is invalid or do not match
     */
    error BalancerPoolConnectorInvalidInputLength();

    /**
     * @dev The given tokens out length is not the expected one
     */
    error BalancerPoolConnectorInvalidTokensOutLength();

    /**
     * @dev The given pool ID and bpt in do not match on Balancer vault
     */
    error BalancerPoolConnectorInvalidPoolId(bytes32 poolId, address bpt);
