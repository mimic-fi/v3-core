...
import { loadOrGetOdosSwapData } from '../helpers/odos'

export function itBehavesLikeOdosV2Connector(
  CHAIN: number,
  USDC: string,
  WETH: string,
  WHALE: string,
  ...
  context('USDC-WETH', () => {
    ...
    it('swaps correctly USDC-WETH', async function () {
      ...
      const data = await loadOrGetOdosSwapData(CHAIN, this.connector, usdc, weth, amountIn, SLIPPAGE)
  ...
  context('WETH-USDC', () => {
    ...
    it('swaps correctly WETH-USDC', async function () {
      ...
      const data = await loadOrGetOdosSwapData(CHAIN, this.connector, weth, usdc, amountIn, SLIPPAGE)
