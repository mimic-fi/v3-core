...
describe('MorphoAaveV2Connector', function () {
  ...
  before('deploy connector', async () => {
    connector = await deploy('MorphoAaveV2Connector', [LENS, REWARDS_DISTRIBUTOR])
  ...
  it('deploys the connector correctly', async () => {
    ...
    expect(await connector.lens()).to.be.equal(LENS)
    expect(await connector.rewardsDistributor()).to.be.equal(REWARDS_DISTRIBUTOR)
  })
  ...
  it('supplies liquidity', async () => {
    ...
    const previousSupplyBalance = await connector.getSupplyBalance(AUSDC)
    ...
    const currentSupplyBalance = await connector.getSupplyBalance(AUSDC)
  ...
  describe('getAToken', () => {
    context('USDC', () => {
      it('returns AUSDC address', async () => {
        expect(await connector.getAToken(USDC)).to.be.equal(AUSDC)
      })
    })
  })
})
